...
public:
   ...
   TestResultRecorder(int _fileHandle) {
      ...
      commentList.Add(StringFormat("VOL=%d/%s(%s)", __config.volumeSettings, EnumToString(__config.volumeSettings), __volumeCalculator.toString()));
      commentList.Add(StringFormat("SPREAD=%d", __config.maxSpread));
      commentList.Add(StringFormat("INC_SWAP=%d", __config.isIncludeSwap));
      commentList.Add(StringFormat("BUY=%d", __config.buyable));
      commentList.Add(StringFormat("SELL=%d", __config.sellable));
      ...
      commentList.Add(StringFormat("ORDER_TIME=%s", StringSubstr(EnumToString(__config.createOrderTimeframe), 7)));
      commentList.Add(StringFormat("ORDER_MA=%d", __config.orderMaPeriod));
      commentList.Add(StringFormat("ORDER_LONG_MA=%d", __config.orderLongMaPeriod));
      commentList.Add(StringFormat("TREND_TIME=%s", StringSubstr(EnumToString(__config.hedgeDirectionTimeframe), 7)));
      commentList.Add(StringFormat("TREND_MA=%d", __config.hedgeMaPeriod));
      commentList.Add(StringFormat("TREND_LONG_MA=%d", __config.hedgeLongMaPeriod));
